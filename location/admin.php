...
for ($i=2;$i<=32;$i=$i+2) $html_ratio .= '<option value="'.$i.'" '.(($i == $selectid)?'selected':'').'>'.(64*$i).'x'.(32*$i).' | '.(22*$i).'x'.(17*$i).'</option>';
return $html_ratio;
}
...
// Buffer OFF 
 switch ($do) {
	...
	$arrGen = new Menager();
	$arrows = $arrGen->arrowsGenerator('index.php?mode=control&do=log&',$curlist,$count,$max,'admin/userf');
	
    if ( $arrows ) {		  
		ob_start(); include MCR_STYLE.'admin/user_find_list.html'; 				  
		$html .= ob_get_clean();			  
	}	
	...
	$search_by = $_GET["sby"];
	$input     = TextBase::MySQLCompatible($_GET['input']);
 ...
 ob_start();
  switch ($do) {
	...
	$and_ban_user = (isset($_POST['banip_anduser']) and (int)$_POST['banip_anduser'])? true : false;
		...
		BD("DELETE FROM {$bd_names['ip_banning']} WHERE IP='".$ban_user->ip()."'");	
		BD("INSERT INTO {$bd_names['ip_banning']} (IP,time_start,ban_until) VALUES ('".$ban_user->ip()."',NOW(),NOW()+INTERVAL $ban_time DAY)");
	...
	case 'cms':
include MCR_STYLE.'admin/cms.html';
break;	
    ...
    break;
case 'theme':
$theme  = TextBase::HTMLDestruct($_POST['theme']);
$site_ways['style']      = $theme   ;
include MCR_STYLE.'admin/theme.html';
break;		
    case 'profile':  
	if ($ban_user) {
	...
	$ip = $_GET['ip']; BD("DELETE FROM {$bd_names['ip_banning']} WHERE IP='$ip'");
