...
elseif ( isset($_GET["user_name"]) )   $tmp_name =  TextBase::MySQLCompatible($_GET['user_name']); 
elseif ( isset($_GET['female']) )      $use_default_skin = true; 
elseif ( !empty($user) )             { $user_name = $user->name(); if ($user->isFemale()) $male = false; $use_default_skin = $user->defaultSkinTrigger(); }
...
if ($use_default_skin or !file_exists($way_skin)) {
